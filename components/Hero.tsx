  ...
  return (
    <section className="relative min-h-[95vh] flex flex-col items-center justify-center pt-28 pb-16 px-6 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-5xl flex flex-col items-center text-center">
        <div className="mb-6 md:mb-10 px-6 py-2 bg-amber-500/10 border border-amber-500/20 rounded-full backdrop-blur-sm inline-flex items-center gap-2">
           <span className="w-2 h-2 bg-amber-500 rounded-full animate-ping"></span>
           <span className="text-amber-500 font-bold text-[9px] md:text-xs tracking-[0.15em] uppercase">
             40 vagas para quem decidiu que 2026 será diferente. Você é um deles?
           </span>
        </div>
        
        <h1 className="text-[2.6rem] md:text-7xl lg:text-8xl font-black mb-8 text-white leading-[1.05] tracking-tight">
          Desperte o <span className="gold-text">Empreendedor</span> que <br className="hidden md:block" /> 
          existe em você.
        </h1>

        <div className="text-base md:text-[1.35rem] text-gray-400 mb-12 max-w-4xl mx-auto leading-[1.6] font-light opacity-95">
          Em 2026, você estará celebrando seu negócio consolidado ou lamentando as chances que deixou passar. A <span className="text-white font-bold italic">Turma 3 da E-Empreenda+ </span> é a sua oportunidade para aprender a <br className="hidden md:block" />
          construir um negócio lucrativo com <span className="text-white font-medium">propósito e visão estratégica.</span>
        </div>
        
        <p className="text-amber-500/80 text-sm md:text-base mb-10 font-medium italic tracking-wide">
        ...
        <div className="w-full flex flex-col items-center gap-8">
          <button 
            onClick={onCtaClick}
            className="w-auto md:w-auto gold-gradient text-gray-950 font-black text-sm md:text-xl px-8 md:px-20 py-4 md:py-6 rounded-xl md:rounded-2xl btn-shadow hover:scale-105 active:scale-95 transition-all duration-300 shadow-[0_20px_60px_rgba(191,149,63,0.4)] tracking-tight md:tracking-normal"
          >
