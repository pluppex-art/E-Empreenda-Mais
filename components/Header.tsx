  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-[#010814]/90 backdrop-blur-md py-3 shadow-2xl border-b border-white/5' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between">
        ...
        <button 
          onClick={onCtaClick}
          className={`gold-gradient text-gray-950 font-extrabold text-[10px] md:text-xs px-5 py-2.5 rounded-full btn-shadow transition-all duration-300 transform ${scrolled ? 'opacity-100 scale-100' : 'opacity-0 scale-90 pointer-events-none md:opacity-100 md:scale-100 md:pointer-events-auto'}`}
        >
          PRÉ-INSCRIÇÃO VIP
        </button>
      </div>
